...
import { useMaintenance } from '../../hooks/useMaintenance'; // Import the new hook

export const LandlordMaintenanceList: React.FC = () => {
    const { user } = useAuth();
    
    // USE THE HOOK: Logic extracted!
    const { requests, isLoading, changeStatus } = useMaintenance(user?.id, user?.role || 'landlord');

    // Helper functions for UI rendering (Badges) stay here as they are purely visual
                            ...
                            requests.map((request) => (
                                <tr key={request.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        {/* Actions use the new changeStatus function from the hook */}
